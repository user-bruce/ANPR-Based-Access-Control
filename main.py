...
def segment_chars(plate_img, fixed_width):
    ...
    #find this block of code, the initial copy is 
    '''
    labels = measure.label(thresh, neighbors=8, background=0)
    '''
    labels = measure.label(thresh, background=0, connectivity=None)
    ...
    for label in np.unique(labels):
        ...
        #ABOVE LINE WAS CHANGED FROM: cnts = cnts[0] if imutils.is_cv2() else cnts[1]
        # ensure at least one contour was found in the mask
        if len(cnts) > 0:

            # grab the largest contour which corresponds to the component in the mask, then
            # grab the bounding box for the contour
            # ORIGINAL LINE OF CODE: c = max(cnts, key=cv2.contourArea)
    ...
    # BELOW LINE WAS CHANGED FROM : _, contours, hier = cv2.findContours(charCandidates, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
...
class PlateFinder:
    ...
    #find this copy of the code
    #original copy of the code is given as shown below, _, 
    '''
    ...
    #find this copy of the code
    #original copy of the code is gicen below
    '''
